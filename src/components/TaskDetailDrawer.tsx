...
import { TASK_PATH, getTaskStepStatus, type TaskStep, type TaskStepStatus } from '@/lib/taskStages'
    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed inset-y-0 right-0 z-50 flex w-full max-w-3xl flex-col overflow-hidden border-l border-border bg-background shadow-2xl data-[state=open]:animate-slide-up focus:outline-none">
          {task && <DrawerBody task={task} onClose={() => onOpenChange(false)} />}
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
  )
...
const priorityToneClass: Record<string, string> = {
  ...
  Normal: 'bg-chart-orange/15 text-chart-orange',
  Low: 'bg-chart-mint/15 text-chart-mint',
}

function DrawerBody({ task, onClose }: { task: Task; onClose: () => void }) {
  const path = TASK_PATH
  const isClosed = task.Status === 'Completed'
  const [pendingStep, setPendingStep] = useState<TaskStep | null>(null)
  const mutation = useUpdateTaskStatus()

  function handleStepClick(step: TaskStep) {
    if (step.apiName === task.Status) return
    setPendingStep(step)
  }
  ...
  function confirmChange() {
    ...
    mutation.mutate(
      { taskId: task.Id, status: target.apiName },
      {
        onSuccess: () => {
          toast({
            variant: 'success',
            title: 'Status actualizado',
            description: `${task.Subject} → ${target.label}`,
          })
          setPendingStep(null)
        },
        onError: (err) => {
  ...
  return (
    <>
      <div className="relative shrink-0 overflow-hidden border-b border-border">
        <div className="absolute inset-x-0 top-0 h-32 bg-gradient-to-r from-chart-cyan/30 via-chart-violet/20 to-chart-blue/30" />
        <div className="absolute -right-8 -top-8 h-44 w-44 rounded-full bg-chart-cyan/20 blur-3xl" />
        <div className="relative flex items-start justify-between p-6">
          <div className="flex items-start gap-4">
            <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border-4 border-card bg-gradient-to-br from-chart-cyan to-chart-violet shadow-xl">
            ...
            <div className="space-y-1.5 pt-1">
              ...
              <div className="flex flex-wrap items-center gap-2 text-xs">
                <span className="inline-flex items-center gap-1 rounded-md bg-chart-cyan/15 px-2 py-0.5 font-medium text-chart-cyan">
                  <Sparkles className="h-3 w-3" /> Task
                </span>
                {task.Owner?.Name && (
                  <span className="text-muted-foreground">
                    Owner: <span className="text-foreground">{task.Owner.Name}</span>
                  </span>
                )}
                <span className="text-muted-foreground">·</span>
                <span className="font-mono text-[10px] text-foreground/60">{task.Id}</span>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6 p-6">
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            <KpiCell
              icon={Calendar}
              label="Due Date"
              value={task.ActivityDate ? formatDate(task.ActivityDate) : '—'}
              tone="cyan"
            />
            <KpiCell
              icon={AlertTriangle}
              label="Priority"
              value={task.Priority}
              tone={task.Priority === 'High' ? 'coral' : task.Priority === 'Low' ? 'mint' : 'orange'}
            />
            <KpiCell
              icon={CheckCircle2}
              label="Status"
              value={task.Status}
              tone={isClosed ? 'mint' : 'blue'}
            />
            <KpiCell
              icon={FileText}
              label="Type"
          ...
          <section className="rounded-2xl border border-border bg-card">
            <div className="flex items-center justify-between border-b border-border p-5">
              <div>
                <h3 className="font-display text-base font-semibold">Estado de la tarea</h3>
                <p className="text-xs text-muted-foreground">
                  Status actual: <span className="font-medium text-foreground">{task.Status}</span>
            ...
            <div className="overflow-x-auto p-5 scrollbar-thin">
              <ol className="flex min-w-max items-stretch">
                {path.map((step, idx) => {
                  const status = getTaskStepStatus(step, task.Status, path)
            ...
            {pendingStep && (
              <div className="border-t border-border bg-chart-cyan/[0.04] p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="text-sm">
                    Cambiar status a{' '}
                    <span className="font-semibold text-chart-cyan">{pendingStep.label}</span>?{' '}
                    {pendingStep.apiName === 'Completed' && (
                      <span className="text-xs text-chart-mint">(Se setea CompletedDateTime)</span>
                    )}
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      disabled={mutation.isPending}
                      className="inline-flex items-center gap-1.5 rounded-lg bg-chart-cyan px-3 py-1.5 text-xs font-semibold text-white shadow-lg shadow-chart-cyan/30 hover:bg-chart-cyan/90 disabled:opacity-50"
                    >
                      {mutation.isPending ? (
                        <>
          ...
          <section className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <DetailCard title="Detalles">
              ...
              <DetailRow
                label="Priority"
                value={task.Priority}
                badge={priorityToneClass[task.Priority]}
              />
            ...
            <DetailCard title="Trazabilidad">
              ...
              <DetailRow
                label="Last Modified"
                value={task.LastModifiedDate ? formatDateTime(task.LastModifiedDate) : '—'}
              />
              <DetailRow
                label="Completed"
                value={task.CompletedDateTime ? formatDateTime(task.CompletedDateTime) : '—'}
              />
            </DetailCard>
          </section>
          ...
          {task.Description && (
            <section className="rounded-2xl border border-border bg-card p-5">
              <div className="flex items-center gap-2">
                <FileText className="h-4 w-4 text-chart-cyan" />
...
function TaskChevron({
  ...
  const variants: Record<TaskStepStatus, { bg: string; icon: React.ReactNode }> = {
    completed: {
      bg: 'bg-chart-mint/20 text-chart-mint',
      icon: <CheckCircle2 className="h-3.5 w-3.5" />,
    },
    current: {
      bg: 'bg-chart-cyan text-white shadow-lg shadow-chart-cyan/40',
  ...
  return (
    <button
      ...
      className={cn(
        ...
        !isFirst && 'pl-7',
        !isLast && 'pr-3',
        clickable && 'cursor-pointer hover:brightness-110 hover:ring-2 hover:ring-chart-cyan/40 focus:outline-none focus:ring-2 focus:ring-chart-cyan/60',
